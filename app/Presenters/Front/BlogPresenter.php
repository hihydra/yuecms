...
Eof;
    ...
    private function articleCategory($category)
    {
        ...
        $str = '<a class="blog-grid-supplemental-category" href="'.url('category/'.$this->encodeId($category->id).'.html').'"> '.$category->name.'</a>,';
        return rtrim($str,',');
    }
...
Eof;
     ...
     * @return [type]                               [description]
     */
    public function postDetailCategories($category)
    {
        $str = '';
        $this->setEncryptConnection('category');
